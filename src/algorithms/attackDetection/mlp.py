...
# 设置环境变量来隐藏 TensorFlow 和 oneDNN 输出
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"  # 隐藏所有 info 和 warning
...
class ModelPredictor:
    def __init__(
        self, mlp_model_path, keras_model_path, onehot_encoder_path, scaler_path
    ):
        """
        初始化模型预测类。
    ...
    def preprocess_data(self, sample):
        ...
        sample_data = pd.concat(
            [sample_data, pd.DataFrame(sample_onehot.toarray())], axis=1
        )
        ...
        # 标准化样例数据
        sample_scaled = self.scaler.transform(
            sample_data
        )  # 使用训练时的 scaler 对样例数据进行标准化
    ...
    def predict(self, sample):
        ...
        prediction_keras = (prediction_keras > 0.5).astype(
            int
        )  # 二分类: 大于0.5预测为1，否者为0
...
if __name__ == "__main__":
    ...
    # 初始化模型预测器
    mlp_model_path = os.path.normpath(
        os.path.join(base_dir, "../../../res/model/MLP/mlp_model_trained.pkl")
    )
    keras_model_path = os.path.normpath(
        os.path.join(base_dir, "../../../res/model/MLP/keras_mlp_model_trained.keras")
    )
    onehot_encoder_path = os.path.normpath(
        os.path.join(base_dir, "../../../res/model/MLP/onehot_encoder.pkl")
    )
    scaler_path = os.path.normpath(
        os.path.join(base_dir, "../../../res/model/MLP/scaler-mlp.pkl")
    )
    ...
    # 评估并输出测试准确率
    test_data_path = input("请输入测试集 CSV 文件路径：")
