...
class Model:
    def __init__(self):
        ...
        self.vectorizer = joblib.load(
            os.path.normpath(
                os.path.join(
                    base_dir, "../../../res/model/SVM-Email/vectorizer-SVM.pkl"
                )
            )
        )  # 加载TF-IDF向量器
        self.model = joblib.load(
            os.path.normpath(
                os.path.join(
                    base_dir, "../../../res/model/SVM-Email/svm-emaildetect.pkl"
                )
            )
        )  # 加载训练好的SVM模型
        self.encoder = joblib.load(
            os.path.normpath(
                os.path.join(base_dir, "../../../res/model/SVM-Email/labelencoder.pkl")
            )
        )  # 加载标签编码器
    ...
    def word_split(self, email):
        ...
        all_words = [
            self.token.stem(word) for word in content if word not in self.stopwords_list
        ]
