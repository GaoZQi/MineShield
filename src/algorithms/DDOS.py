...
class GBDTPredictor:
    def __init__(self, data_path=None):
        ...
        """
        # 固定模型路径
        self.model_path = '../../res/model/ddos_gbdt_model.pkl'

        # 如果没有提供数据集路径，使用默认路径
        self.data_path = data_path if data_path else '../../res/data/DDos/DDoS_dataset.csv'
    ...
    def predict(self):
        ...
        # 6. 获取每个样本的 DDoS 攻击概率
        y_prob = self.model.predict_proba(X_scaled)[:, 1]  # 获取每个样本属于类别 1（DDoS 攻击）的概率

        # 7. 将预测概率添加到原数据中
        data['ddos_probability'] = y_prob  # 添加 DDoS 攻击的概率列

        # 8. 保存结果到 CSV 文件
        result_path = os.path.join(os.path.dirname(self.data_path), 'prediction_with_probabilities.csv')
